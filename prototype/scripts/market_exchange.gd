...
func _draw() -> void:
	...
	if receiving:
		...
		if font:
			...
			draw_string(font, Vector2(-48, 24), "实付%d币" % paid, HORIZONTAL_ALIGNMENT_LEFT, -1, 23, Color("22392e"))
	else:
		for i in 5:
		...
		if font: draw_string(font, Vector2(840, 728), "付清 %d币" % paid, HORIZONTAL_ALIGNMENT_CENTER, 240, 26, Color("f4eddd"))
