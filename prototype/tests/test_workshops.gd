...
func run() -> void:
	...
	verify(t.choose(0) and t.stage == "loading_work", "Carry sealed cargo to dock for counting")
	for i in 3: verify(t.choose(0), "Load counted batch")
	verify(t.stage == "dock" and t.loaded_count() == 10, "All ten crates precede route selection")
