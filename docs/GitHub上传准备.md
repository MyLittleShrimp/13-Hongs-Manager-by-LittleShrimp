...
当前发布目标为 **v0.8.0**，包含金额单位「币」、14张晴雨背景、4幅离线历史参考图、触摸兼容修复、两首背景音乐、茶市剧情和验茶／复焙演出。游戏版本号统一读取 `prototype/project.godot` 的 `config/version`。发布说明位于 `docs/releases/v0.8.0.md`。
...
该脚本从当前Git提交构建干净副本，加入便携引擎，检查两首音乐并导入资源；打包后重新解压，对成品包运行音乐循环、角色交易、工序演出、天气与历史图像、GPU启动检查。成功产物位于 `output/releases/v<版本号>/`，包括Windows完整ZIP、SHA256校验、构建记录和发布说明。脚本不会自动上传或覆盖已有ZIP。
...
- 分支：`main`。
- 纳入：Godot工程与脚本、30张ImageGen游戏美术与4张历史参考图、导入配置、测试、文档、素材提示词、启动器和可重复下载工具。
